...
public class GenePanelServiceImpl implements GenePanelService {
    ...
    private List<GenePanelData> createGenePanelData(MultiKeyMap genePanelDataMap, List<String> molecularProfileIds,
        ...
        List<String> studyIds = molecularProfileIds
                .stream()
                .map(molecularProfileMap::get) // assuming all molecular profiles are valid. invalid id would throw runtime exception
                .map(MolecularProfile::getCancerStudyIdentifier)
                .collect(Collectors.toList());

        MultiKeyMap samples = createSampleMap(studyIds, sampleIds);
        
        List<String> sequencedSampleListIds = studyIds
                .stream()
                .distinct()
    ...
    private List<GenePanelData> createGenePanelDatabyPatientIds(
        ...
        Map<Pair<String, String>, List<Sample>> sampleSet = createSampleMapByPatientId(studyIds, patinetIds);
        ...
        for(int i = 0; i < patinetIds.size(); i++) {
            String patinetId = patinetIds.get(i);
            String molecularProfileId = molecularProfileIds.get(i);
            MolecularProfile molecularProfile = molecularProfileMap.get(molecularProfileId);
            String studyId = molecularProfile.getCancerStudyIdentifier();
